...
async function notifyWhatsApp(lead: LeadPayload) {
  ...
  const body = [
    "🏠 *New Lead — DLF Privana Amraj Infra*",
    `*Name:* ${lead.name}`,
    `*Phone:* ${lead.phone}`,
...
async function notifyEmail(lead: LeadPayload) {
  ...
  const html = `
    <h2 style="font-family:Georgia,serif;color:#0B1F3A">New Lead — DLF Privana Amraj Infra</h2>
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "DLF Privana Amraj Infra <leads@amrajinfra.in>",
      to: [toEmail],
