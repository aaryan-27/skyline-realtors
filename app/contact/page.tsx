...
export const metadata: Metadata = {
  ...
  description: `Contact Skyline Realtors — call ${SITE.phone}, WhatsApp, or visit our office at ${SITE.address.full}. Book a free consultation or site visit today.`,
            ...
            <div className="mt-4 grid grid-cols-2 gap-4">
              {OFFICE_IMAGES.map((src, i) => (
                <div key={i} className="relative aspect-[4/3] overflow-hidden rounded-2xl">
                  <Image src={src} alt={`Skyline Realtors office ${i + 1}`} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover" />
                </div>
              ))}
            </div>
          </Reveal>
      ...
      <section className="bg-white pb-20">
        <div className="container">
          <div className="overflow-hidden rounded-3xl border border-navy/10 shadow-card">
            <iframe
              title="Skyline Realtors office location"
              src={SITE.mapsEmbed}
              width="100%"
              height="460"
              loading="lazy"
